...
namespace AP.Model
{
    public class ReservationHebergement : Bdd
    {
        ...
        public ReservationHebergement GetReservationEnCours(int idHebergement)
        {
            ...
            _bdd.Open();
            MySqlCommand query = _bdd.CreateCommand();
            query.CommandText = "SELECT hebergement.idHebergement, hebergement.libelle, hebergement.adresse, hebergement.description, hebergement.idVille, hebergement.latitude, hebergement.longitude, hebergement.prix, hebergement.uuid, hebergement.idUtilisateur, hebergement.dateEnregistrement, hebergement.actif , villes.idVille, villes.libelle, villes.code_postal, villes.latitude, villes.longitude, villes.idRegion, villes.description, villes.uuid, regions.idRegion, regions.libelle, regions.latitude, regions.longitude, regions.lv_zoom, regions.description, u2.idUtilisateur, u2.email, u2.mdp, u2.nom, u2.prenom, u2.idRole, u2.acceptRGPD, u2.dateAcceptRGPD, u2.DoB, u2.token, ru2.idRole, ru2.libelle, reservations_hebergement.*, u1.idUtilisateur, u1.email, u1.mdp, u1.nom, u1.prenom, u1.idRole, u1.acceptRGPD, u1.dateAcceptRGPD, u1.DoB, u1.token, ru1.idRole, ru1.libelle FROM hebergement INNER JOIN villes USING(idVille) INNER JOIN reservations_hebergement USING(idHebergement) INNER JOIN utilisateurs u1 ON u1.idUtilisateur = reservations_hebergement.idUtilisateur INNER JOIN roles ru1 ON u1.idRole = ru1.idRole INNER JOIN utilisateurs u2 ON u2.idUtilisateur = hebergement.idUtilisateur INNER JOIN roles ru2 ON u2.idRole = ru2.idRole INNER JOIN regions USING(idRegion) INNER JOIN reservations_voyages ON reservations_hebergement.idVoyage = reservations_voyages.idReservationVoyage WHERE idHebergement = @idHebergement AND dateDebut < NOW() AND dateFin > NOW()";
            query.Parameters.AddWithValue("@idHebergement", idHebergement);
            MySqlDataReader reader = query.ExecuteReader();
            while (reader.Read())
            {
