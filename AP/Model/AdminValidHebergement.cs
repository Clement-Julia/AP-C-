...
namespace AP.Model
{
    public class AdminValidHebergement : Bdd
    {
        ...
        public AdminValidHebergement(int IdUtilisateur = 0)
        {
            if (IdUtilisateur != 0)
            {
                ...
                while (reader.Read())
                {
                    ...
                    this.Latitude = reader.GetFloat(4);
                    this.Longitude = reader.GetFloat(5);
                    this.Prix = reader.GetInt32(6);
                    this.IdUtilisateur = reader.GetInt32(7);
                    this.DateEnregistrement = reader.GetDateTime(8);
                    this.IsActif = reader.GetBoolean(9);
                }
                _bdd.Close();
            }
        }

        public void InitialisationAdminValidHebergement(int id, string libelle, string description, string nomVille, float latitude, float longitude, decimal prix, int idUtilisateur, DateTime dateEnregistrement, Boolean isActif)
        {
        ...
        public Boolean InsertAdminValidHebergement(string libelle, string description, string nomVille, float latitude, float longitude, decimal prix, int idUtilisateur)
        {
            Boolean error = false;
            ...
            try
            {
                _bdd.Open();
                MySqlCommand query = _bdd.CreateCommand();
                query.CommandText = "INSERT INTO admin_valid_hebergements (libelle, description, nomVille, latitude, longitude, prix, idUtilisateur, dateEnregistrement, is_actif) VALUES(@libelle, @description, @nomVille, @latitude, @longitude, @prix, @idUtilisateur, NOW(), 1)";
